...
const Sidebar = () => {
    ...
    React.useEffect(() => {
        const fetchContent = async () => {
            try {
                const response = await fetch('/api/root/content');
                if (response.ok) {
                            ...
                            {dynamicContent.profiles.map(profile => (
                                <li key={profile._id}>
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            handleRestrictedAccess(`/newsletters/pdf?url=${encodeURIComponent(`http://localhost:5000${profile.fileUrl}`)}`);
                                        }}
                                    >
                                        {profile.year}
                                    </Link>
                        ...
                        <ul className="nested-list">
                            ...
                            {dynamicContent.newsletters.map(newsletter => (
                                <li key={newsletter._id}>
                                    <Link to={`/newsletters/pdf?url=${encodeURIComponent(`http://localhost:5000${newsletter.fileUrl}`)}`}>
                                        {newsletter.year}
                                    </Link>
                                </li>
                            ))}
